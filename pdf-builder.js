// PDF BUILDER — Atelier B1: haben/sein + zu + Infinitiv
(function () {
    ...
    function buildCast() {
        return `<div class="cast-banner">
            <h4>👋 Cast „Annettes Deutschkurs" — Atelier B1: haben/sein + zu + Infinitiv</h4>
            <div class="cast-grid">
    ...
    function buildTheory() {
        ...
        return `<h1 class="chapter">📘 1. Teorie — haben/sein + zu + Infinitiv</h1>` + t;
    }
    ...
    function translateBlock(title, instruction, data) {
        ...
        data.forEach(it => { h += `<tr><td class="ro-text">${it.ro}</td><td><div class="write-line"></div></td></tr>`; });
        h += `</tbody></table></div>`;
        return h;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillInBlock('Übung 1 — haben + zu + Infinitiv (obligație activă) 🔵', 'habe · hast · hat · haben · habt · haben. Infinitivul cu zu la final. ≈ müssen.', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += fillInBlock('Übung 2 — sein + zu + Infinitiv (pasiv) 🟢', 'Singular → ist, plural → sind. ≈ können/müssen + Passiv.', ex2Data);
        }
        ...
        let html = `<h1 class="chapter new-section">🔁 4. Verbele atelierului — Präsens · Präteritum · Perfekt</h1>
            <div class="andreea-note">
                <div class="andreea-note-content">
                    ...
                    <div>Vedetele sunt haben și sein — verbele care formează structura: Ich habe zu… (activ) și Das ist zu… (pasiv). Restul apar des în ea: erledigen, ausfüllen-separabil (→ auszufüllen), tun-TARE (viel zu tun) și bezahlen. La haben+zu TU acționezi; la sein+zu, lucrului i se face ceva. Toate verificate pe PONS. Präteritum = imperfect.</div>
                </div>
            </div>`;
